...
매물번호든, 매물 페이지 링크든, 말로 설명이든, 사진 폴더든 — **뭘 던져도
알아서 알아듣고 시작합니다.**

## 한 장으로 보는 전체 흐름 — 노드 25개, 실행 순서 그대로

<img src="naver-realtor-blog-pro/docs/node-map.png" width="640" alt="자동화 노드 25개 세로 지도: 셋업 3개(처음 한 번만) → 입력과 수집 → 원고와 이미지 → 네이버로 옮겨 적기 → 마무리">

위에서 아래로 실제로 일어나는 순서 그대로입니다. 맨 위 셋업 3개는 처음
한 번만 — 그 뒤로는 매 실행이 04번(매물 입력)부터 돕니다. 색은 "누가 하느냐"입니다:
**주황은 사장님 손이 필요한 순간(25개 중 5곳뿐)**, 실선은 코덱스 AI,
점선은 프로그램(AI 비용 0)입니다. 마지막에 **임시저장**하고 멈추며,
발행(25번)은 다시 사장님에게 돌아옵니다.

> 같은 내용을 더 크게 보고 싶으면
> [docs/node-map.html](naver-realtor-blog-pro/docs/node-map.html)을
> 브라우저로 여세요.
...
**명령어를 칠 필요가 없습니다.** 처음 매물 글을 시키면 도구가 로그인이
필요한 걸 알아채고 **알아서 브라우저 창을 띄워줍니다.** 사장님은 그 창에서
평소처럼 네이버에 로그인만 하면 됩니다 (`로그인 상태 유지`에 체크하세요).
로그인이 확인되면 창이 저절로 닫힌 뒤 같은 전용 브라우저가 한 번 더 열려
로그인 상태가 실제로 유지되는지 확인하고 하던 작업이 이어집니다.

이 로그인은 도구 전용 브라우저에 약 2주간 유지되므로, 그동안은 매물 글을
몇 개를 만들든 다시 로그인할 일이 없습니다. 2주쯤 지나 만료되면 도구가
다시 로그인 창을 띄워줍니다. 아이디·비밀번호는 도구를 거치지 않습니다 —
네이버 화면에 직접 입력하는 것뿐입니다.

### ② 사무소 정보 (딱 한 번) — 블로그 아이디까지 같이 주세요
...
**블로그 아이디를 같이 주는 이유** — 도구가 글을 옮겨 적을 글쓰기 화면
주소(`blog.naver.com/아이디`)를 여는 데 필요합니다. 안 주면 그 단계에서
한 번 물어보느라 멈춥니다. 처음에 같이 주면 끝까지 안 멈춥니다.

**내 블로그 아이디 찾는 법** — 보통 네이버 아이디(@naver.com 메일의 @ 앞부분)와
같습니다. 블로그 주소를 바꾼 적이 있다면 다를 수 있으니, 확실하게는
내 블로그를 열고 주소창의 `blog.naver.com/` 뒤에 보이는 글자를 쓰면 됩니다.

도구가 이 정보를 설정 파일에 적어두고, **다음 매물부터는 다시 묻지 않습니다.**
"상담 전화는 지난번이랑 같아"라고만 해도 알아서 찾아 씁니다. 안 적고
시작해도 괜찮습니다 — 시작할 때 딱 한 번 물어보고, 답하면 블로그
아이디까지 같이 저장돼서 그 뒤로는 안 묻습니다. 이 정보는 썸네일의
사무소명, 전화 배너의 번호, 글 끝의 전화 링크에 쓰입니다.

## 매물 글 하나 만들기 — 한 번에 다 넣으면 안 물어보고 끝까지 갑니다

첫 메시지에 아래 세 가지를 담으면, 도구가 **중간에 한 번도 안 묻고**
임시저장까지 완주합니다:

1. **매물** — 네이버 매물번호 하나, 또는 매물 페이지 주소, 또는 말로 설명
2. **사진** — 사진 폴더 위치 (네이버 번호를 줬다면 생략 가능 — 알아서 다 가져옴)
3. **블로그 아이디** — 처음 한 번만 (사무소 정보도 처음 한 번만)

복사해서 채워 쓰는 템플릿:

> 블로그 글 만들어서 임시저장까지 해줘.
> 매물: ______ (번호 / 주소 / 설명 아무거나)
> 사진: ______ (폴더 위치, 네이버 번호면 생략)
> 블로그: ______ (첫 회만)
...
실제 예시 — 네이버 매물번호가 있는 경우 (이게 제일 짧습니다):

> "매물번호 2645188091 이걸로 블로그 글 만들어서 임시저장까지 해줘.
> 내가 중개하는 매물이야."

실제 예시 — 매물 페이지 링크를 붙여넣는 경우:

> "이 매물 링크로 블로그 글 만들어서 임시저장까지 해줘. 내가 중개하는 매물이야.
> https://fin.land.naver.com/articles/2645188091"

빠뜨린 게 있어도 괜찮습니다 — 도구가 **시작할 때 한 번에 모아서** 묻고,
그 뒤로는 끝까지 혼자 갑니다. 사실과 사진이 서로 다른 곳에서 와도
(예: 정보는 카톡 캡처, 사진은 폴더) 알아서 한 판으로 합칩니다.

## 사진은 알아서 알아봅니다

사진 폴더 위치만 알려주면 됩니다. **AI가 원고를 쓰기 전에 사진을 한 장씩
직접 열어 봅니다** — 이게 외관인지, 층고 높은 내부인지, 사무실인지,
화장실인지를 눈으로 확인하고 사진마다 한 줄 메모(라벨)를 만듭니다. 그리고
그 메모를 기준으로:

- **챕터가 정해집니다** — 화장실 사진이 있으면 화장실 문단이 생기고, 없는
  공간의 문단은 만들지 않습니다
- **사진이 제자리에 놓입니다** — 내부 층고를 설명하는 문장 바로 뒤에 내부
  사진이, 위치 문장 아래에 지도가 붙습니다
- **주신 사진은 기본적으로 전부 싣습니다** — 폴더째 줬다는 건 다 실으라는
  뜻이니까요. 같은 공간 사진이 여러 장이면 그 문단 뒤에 연달아 배치합니다
  (외관 4장이면 외관 문단 뒤에 4장 연속). 사실상 같은 컷이거나 심하게
  흔들린 사진만 빼고, 뺀 사진은 이유와 함께 보고합니다
- "대표 사진만 골라 써줘"라고 하면 그때는 알아서 추립니다

사진이 뭘 찍은 건지 사장님이 직접 한 줄씩 적어줘도 됩니다 — 그 경우 사장님
메모가 우선입니다 (매물은 사장님이 제일 잘 아니까요). 뭔지 애매한 사진은
AI가 보수적으로 다루고, 사진에서 확인 안 되는 내용을 지어 쓰지 않습니다.

## 실제로 만들어지는 이미지

말 몇 마디로 시작했지만, 글에는 이런 이미지가 자동으로 들어갑니다.
**코덱스에 기본 내장된 이미지 생성 기능(imagen 스킬)** 으로 그리는 것이라
별도 설치나 추가 결제가 없습니다. 아래는 실제 테스트(파주 상지석동 창고
매매)에서 AI가 만든 실물입니다:

**썸네일** — 글 맨 위. 매물명과 사무소명이 박혀 목록에서 눈에 띕니다:

<img src="naver-realtor-blog-pro/docs/examples/example-thumbnail.png" width="420" alt="썸네일 예시: 파주 상지석동 공장·창고 매매, 포동 공인중개사 민경식">

**전화 상담 배너** — 글 맨 아래. 발행 후 휴대폰에서 이 배너를 탭하면
바로 전화가 걸립니다 (배너 이미지와 그 위의 전화번호 글자 모두):

<img src="naver-realtor-blog-pro/docs/examples/example-cta-banner.png" width="560" alt="전화 상담 배너 예시: 전화 상담 010-9705-8427, 포동 공인중개사 민경식">
...
내 매물의 **매물번호**는 네이버부동산 매물 상세 페이지의 **'기본 정보' 표
맨 아래**(입주가능일 다음 줄)에 있습니다. 그 번호만 주면 도구가 가격·면적
같은 매물 사실과 **등록된 갤러리 사진 전부**를 자동으로 가져옵니다 —
대표 사진 한 장이 아니라, 올려두신 사진 몇 장이든 전부입니다. 몇 장을
찾아서 몇 장을 받았는지 숫자로 보고하고, 못 받은 게 있으면 이유까지
말해줍니다. (지도 공유 링크에는 매물번호가 안 들어있어서, 지도 링크를
주시면 상세 페이지 링크나 번호를 한 번 되물어봅니다.)
...
크게 적힌 가로 배너)를 AI가 새로 그립니다. 그리는 도구는 **코덱스에 기본
내장된 이미지 생성 기능(imagen)** 입니다 — 별도 프로그램을 깔거나 디자인
외주를 주거나 추가 결제를 할 일이 없고, 쓰시는 코덱스 요금제 안에서
만들어집니다. 배너는 한 번 만들면 다음 글부터 재사용합니다. 이미지가
실패해도 글쓰기는 멈추지 않습니다.

**4. 원고 작성** — 쓰기 전에 먼저 **사진을 전부 열어 보고** 사진마다 한 줄
메모를 만듭니다(위 "사진은 알아서 알아봅니다" 참고). 그다음 제목 → 도입 →
매물에 맞는 본문 챕터 2–4개 → 위치 → 핵심 조건 표 → 상담 안내 → 태그
순서로 원고를 씁니다. 핵심은 **챕터가 매물을 따라간다**는 것입니다. 창고면 "물류 진입과 주차·내부 홀" 같은 챕터가 생기고,
...
## 15번 "기계 검증 + 원고 잠금"은 특히 중요해서 따로 설명합니다

AI가 원고를 다 쓴 직후, 네이버로 옮기기 전에 **검사 프로그램**
(`validate-draft.mjs`)이 원고를 봅니다. 로컬 코드라 비용 0, 1초 안에
끝납니다. 검사하는 것:

- 제목 존재 / 한 줄에 완결 문장 하나 / 문단 사이 빈 줄 / 100자 초과 문장
- `위치` 장과 `지도:` 앵커 존재 (앵커에 호수가 들어가면 경고)
- 근거 없는 "검색량 높음·상위노출" 류 과장 표현 차단
- 핵심 조건 표 문법, 강조 남용(볼드 챕터당 1곳·형광 글 전체 2곳 초과)

**실패하면 AI가 딱 한 번 고치고 재검사합니다.** 상한이 1회인 이유 —
고쳤다가 다른 걸 깨뜨리고 또 고치는 무한 루프를 막기 위해서입니다.
두 번째도 실패하면 실패로 정직하게 보고하고 멈춥니다.

**왜 AI가 아니라 코드가 검사하나** — AI에게 "네 글 검토해봐"라고 하면
그날 컨디션에 따라 결과가 다릅니다. 코드는 매번 같은 잣대라 품질의
하한선이 운이 아니라 규칙으로 보장됩니다. 검사 기준을 바꾸고 싶으면
이 파일 하나만 고치면 됩니다 — 커스터마이징 지점이기도 합니다.

**원고 잠금** — 검사를 통과한 `blog-post.md`는 그 순간부터 유일한
원본입니다. 전송 프로그램은 이 파일을 읽기만 하고, 네이버 화면에서
뭔가 고치는 일은 없습니다. 문제가 보이면 화면이 아니라 **파일을 고치고
→ 재검사 → 다시 전송**합니다. 화면에서 고치는 순간 파일과 화면이
갈라져 방금 한 검사가 무의미해지기 때문입니다. 덕분에 전송이 어디서
끊겨도 원고는 파일로 온전히 남아, "다시 해줘" 한 마디면 재시도 비용이
0입니다.

참고로 22번 "저장 전 검사"와는 층이 다릅니다 — 15번은 **원고 자체**
(내용·형식)를 검사하고, 22번은 네이버에 **옮겨진 결과물**(사진·표·지도
개수, 전화 링크)을 셉니다. 글이 좋은지 한 번, 옮기다 잃은 게 없는지
한 번 — 이중 검사입니다.
