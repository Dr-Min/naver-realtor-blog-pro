...
## 한 장으로 보는 전체 흐름 — 노드 24개, 실행 순서 그대로

<img src="naver-realtor-blog-pro/docs/node-map.png" width="640" alt="자동화 노드 24개 세로 지도: 셋업 3개(처음 한 번만) → 입력과 수집 → 원고와 이미지 → 네이버로 옮겨 적기 → 마무리">

위에서 아래로 실제로 일어나는 순서 그대로입니다. 맨 위 셋업 3개는 처음
한 번만 — 그 뒤로는 매 실행이 04번부터 돕니다. 색은 "누가 하느냐"입니다:
**주황은 사장님 손이 필요한 순간(24개 중 4곳뿐)**, 실선은 코덱스 AI,
점선은 프로그램(AI 비용 0)입니다. 마지막에 **임시저장**하고 멈추며,
발행(24번)은 다시 사장님에게 돌아옵니다.
...
## 14번 "기계 검증 + 원고 잠금"은 특히 중요해서 따로 설명합니다
...
참고로 21번 "저장 전 검사"와는 층이 다릅니다 — 14번은 **원고 자체**
(내용·형식)를 검사하고, 21번은 네이버에 **옮겨진 결과물**(사진·표·지도
